...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		if (error == -1)
		{
			MessageBox(hwnd, TEXT("Proxy/Stub Dll Cannot Be Registered."), TEXT("Error"), MB_OK);
			DestroyWindow(hwnd);
			exit(0);
		}
		...
		if (FAILED(hr))
		{
			MessageBox(hwnd, TEXT("COM library cannot be initialized"), TEXT("Error"), MB_OK);
			DestroyWindow(hwnd);
			exit(0);
		}
		...
		if (FAILED(hr))
		{
			MessageBox(hwnd, TEXT("SumSubtract Component Cannot Be Initialized."), TEXT("Error"), MB_OK);
			DestroyWindow(hwnd);
			exit(0);
		}
		...
		wsprintf(str, TEXT("Sum of %d and %d is %d"), n1, n3, n3);
		...
		MessageBox(hwnd, str, TEXT("SUBTRACTIONS"), MB_OK);
		DestroyWindow(hwnd);
		break;
	...
	case WM_DESTROY:
		...
		if (error == -1)
			MessageBox(hwnd, TEXT("Proxy/Stub Dll Cannot Be Un-Registered"), TEXT("Error"), MB_OK);
